...
void validateLoop(const ngraph::Node& node) {
    const auto& loop = dynamic_cast<const ngraph::opset5::Loop&>(node);
    ...
    const auto& executionCondition = ngraph::as_type_ptr<ngraph::opset5::Constant>(loop.input_value(1).get_node_shared_ptr());
...
void dynamicToStaticShapeLoop(std::shared_ptr<ngraph::Node> node) {
    const auto& loop = ngraph::as_type_ptr<ngraph::opset5::Loop>(node);
    VPU_THROW_UNLESS(loop != nullptr, "Encountered node {}, but expected loop", node);

    const auto copied = ngraph::as_type_ptr<ngraph::opset5::Loop>(loop->clone_with_new_inputs(loop->input_values()));
    ...
    for (const auto& outputDescription : loop->get_output_descriptions()) {
        ...
        if (const auto& concatOutputDescription = ngraph::as_type_ptr<ngraph::op::util::SubGraphOp::ConcatOutputDescription>(outputDescription)) {
            ...
            const auto outputShape = std::make_shared<ngraph::opset5::ScatterElementsUpdate>(
                std::make_shared<ngraph::opset5::Constant>(ngraph::element::i64, ngraph::Shape{shape.size()}, shape),
                std::make_shared<ngraph::opset5::Constant>(ngraph::element::i64, ngraph::Shape{1}, axis),
                iterationsCount,
                std::make_shared<ngraph::opset5::Constant>(ngraph::element::i64, ngraph::Shape{}, 0));
