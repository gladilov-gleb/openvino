...
private:
    ...
    static const std::unordered_map<ngraph::DiscreteTypeInfo, Functor>& getSlicers() {
        static const std::unordered_map<ngraph::DiscreteTypeInfo, Functor>& slicers = {
            ...
            {ngraph::opset5::Add::type_info,              sliceBinaryEltwise<ngraph::opset5::Add>},
            {ngraph::opset5::Multiply::type_info,         sliceBinaryEltwise<ngraph::opset5::Multiply>},
            {ngraph::opset5::Minimum::type_info,          sliceBinaryEltwise<ngraph::opset5::Minimum>},
            {ngraph::opset5::Maximum::type_info,          sliceBinaryEltwise<ngraph::opset5::Maximum>},
...
SubGraph getLeaves(ngraph::Node* source, const Nodes& blackList, Functor&& getNext) {
    const auto isOk = [&blackList](ngraph::Node* node) { return Slicers::slice(*node).hasSlice() && !blackList.count(node); };
    Nodes leaves;
...
void getLeavesLCA(ngraph::Node* source, ngraph::Node*& leavesLCA, Nodes& nodes, const Nodes& leaves, const Nodes& allBackward,
    ...
    bfs(
        ...
        [getNextBackward, &allBackward, &prevNodes](const ngraph::Node* current) {
            ...
            return std::count_if(prevNodes.cbegin(), prevNodes.cend(), [&allBackward](ngraph::Node* prev) { return !allBackward.count(prev); });
        },
        [&](ngraph::Node* current) {
            if (current == source) {
                return true;
    ...
    leavesLCA = *lcaCandidates.begin();
    nodes = getNodes(source, leavesLCA, getNextForward);
}
        ...
        if (nodes.count(currentInput)) {
            ...
            const auto adjacentDiff = std::adjacent_find(targetInputs.cbegin(), targetInputs.cend(),
                [](const ngraph::Input<ngraph::Node>& lhs, const ngraph::Input<ngraph::Node>& rhs) {
                    ...
                    if (!lhsSplitConfig.hasSlice() || !rhsSplitConfig.hasSlice()) {
                        return true;
                    }
...
bool ExtractBatch::run_on_function(std::shared_ptr<ngraph::Function> functionPointer) {
    ...
    for (auto currentSource = sources.begin(); currentSource != sources.end(); currentSource = sources.erase(currentSource)) {
        ...
        if (!Slicers::slice(*source).hasSlice()) {
            continue;
        }
        ...
        do {
            getLeavesLCA(source, top, topSubGraph.leaves, topSubGraph.leaves, bottomSubGraph.all, getNextTop, getNextBottom);
